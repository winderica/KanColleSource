const function267 = function (t, e, i) {
    "use strict";
    var n = this && this.__extends || function () {
        var t = Object.setPrototypeOf || { __proto__: [] } instanceof Array && function (t, e) {
            t.__proto__ = e
        } || function (t, e) {
            for (var i in e) e.hasOwnProperty(i) && (t[i] = e[i])
        };
        return function (e, i) {
            function n() {
                this.constructor = e
            }

            t(e, i), e.prototype = null === i ? Object.create(i) : (n.prototype = i.prototype, new n)
        }
    }();
    Object.defineProperty(e, "__esModule", { value: !0 });
    var o = i(0), r = i(2), s = i(77), a = i(7), _ = i(28), u = function () {
        function t(t, e) {
            var i = this;
            this._onImageLoadComplete = function (t, e) {
                null != t && i._cb_onLoadCompleteResources(t.resources), null != i._cb_onTaskComplete && i._cb_onTaskComplete(), i._cb_onLoadCompleteInfo = null, i._cb_onLoadCompleteResources = null, i._cb_onTaskComplete = null, i._keys = null
            }, this._cb_onLoadCompleteInfo = t, this._cb_onLoadCompleteResources = e, this._keys = []
        }

        return Object.defineProperty(t.prototype, "count", {
            get: function () {
                return this._keys.length
            }, enumerable: !0, configurable: !0
        }), t.prototype.add = function (t) {
            return -1 == this._keys.indexOf(t) && this._keys.push(t), this
        }, t.prototype.load = function (t) {
            var e = this;
            void 0 === t && (t = null), this._cb_onTaskComplete = t;
            for (var i = {}, n = new _.SerialTask, o = 0, r = this._keys; o < r.length; o++) {
                var s = r[o];
                n.add(new h(s, i))
            }
            n.start(function () {
                e._onInfoLoadComplete(i)
            })
        }, t
    }();
    e.GaugeLoader = u;
    var l = function (t) {
        function e() {
            return null !== t && t.apply(this, arguments) || this
        }

        return n(e, t), e.prototype._onInfoLoadComplete = function (t) {
            var e = new PIXI.loaders.Loader(o.default.settings.path_root), i = [];
            for (var n in t) {
                var r = t[n];
                this._cb_onLoadCompleteInfo(n, r);
                var s = r.image_path;
                -1 == i.indexOf(s) && (i.push(s), e.add(s, "resources/gauge/" + s + ".png")), s = r.image_light_path, -1 == i.indexOf(s) && (i.push(s), e.add(s, "resources/gauge/" + s + ".png"))
            }
            0 == i.length ? this._onImageLoadComplete(null, null) : e.load(this._onImageLoadComplete)
        }, e
    }(u);
    e.HorizontalGaugeLoader = l;
    var c = function (t) {
        function e() {
            return null !== t && t.apply(this, arguments) || this
        }

        return n(e, t), e.prototype._onInfoLoadComplete = function (t) {
            var e = new PIXI.loaders.Loader(o.default.settings.path_root), i = [];
            for (var n in t) {
                var r = t[n];
                this._cb_onLoadCompleteInfo(n, r);
                var s = r.vertical;
                if (null != s) {
                    var a = s.image_path;
                    -1 == i.indexOf(a) && (i.push(a), e.add(a, "resources/gauge/" + a + ".png")), a = r.vertical.image_light_path, -1 == i.indexOf(a) && (i.push(a), e.add(a, "resources/gauge/" + a + ".png"))
                }
            }
            0 == i.length ? this._onImageLoadComplete(null, null) : e.load(this._onImageLoadComplete)
        }, e
    }(u);
    e.VerticalGaugeLoader = c;
    var h = function (t) {
        function e(e, i) {
            var n = t.call(this) || this;
            return n._onLoadComplete = function (t) {
                if (200 == a.ObjUtil.getNumber(t, "status")) {
                    var e = a.ObjUtil.getObject(t, "data");
                    null != e && (n._dic[n._key] = new s.GaugeSetModel(e)), n._endTask()
                } else n._failedEnd()
            }, n._onLoadFailed = function (t) {
                n._failedEnd()
            }, n._key = e, n._dic = i, n
        }

        return n(e, t), e.prototype._start = function () {
            var t = o.default.settings.path_root + "resources/gauge/" + this._key + ".json";
            axios.get(t).then(this._onLoadComplete).catch(this._onLoadFailed)
        }, e
    }(r.TaskBase)
}